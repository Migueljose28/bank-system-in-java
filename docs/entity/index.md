...
### Regra de negocio
Client (tipo = INDIVIDUAL) → tem dados de CPF

Client (tipo = BUSINESS) → tem dados de CNPJ
...
Taxas (fee)
a taxa será calculadas em cima do valor da transações, podemos cobrar 0,80 de
cada transações. fee = amount - 0,8
- valor fixo evitar calcular e reproduz valores quebrados demais.
